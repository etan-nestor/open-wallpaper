  ...
  const loadFavorites = useCallback(async (showRefreshIndicator = false) => {
    try {
      ...
      const validFavorites = favoritesResults
        ...
        .filter(
          (wallpaper): wallpaper is NonNullable<typeof wallpaper> =>
            wallpaper !== null
        );

      // Adapter les wallpapers pour correspondre à FavoriteWallpaper
      const favoriteWallpapers: FavoriteWallpaper[] = validFavorites.map(
        (w) => ({
          id: w.id,
          optimizedUrl: w.optimizedUrl,
          thumbnailUrl: w.thumbnailUrl,
          category: w.categoryId ?? "Autre",
          title: w.title,
      ...
      const validIds = favoriteWallpapers.map((w) => w.id);
        ...
        { text: "Annuler", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              setFavorites((prev) =>
                prev.filter((item) => item.id !== wallpaper.id)
              );
        ...
        <View style={[styles.favoriteCard, { backgroundColor: theme.card }]}>
          ...
          <View style={styles.favoriteInfo}>
            <View style={styles.textInfo}>
              <Text
                style={[styles.favoriteTitle, { color: "white" }]}
                numberOfLines={1}
              >
                {item.title || "❤️"}
              </Text>
        ...
        {error ? (
          ...
          >
            ...
            <Text style={[styles.emptySubtext, { color: theme.textSecondary }]}>
              Ajoutez des wallpapers à vos favoris en cliquant sur l&apos;icône
              cœur
            </Text>
          </Animated.View>
        ) : (
